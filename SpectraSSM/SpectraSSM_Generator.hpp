...
namespace SpectraSSM {
    ...
    /*
     * @class 状态空间递归生成器
     * @brief 基于频域MoE模型的高性能序列生成器（组合方式）
     *
     * 修改说明：
     * 1. 不再继承频域MoE模型，而是通过参数传入的方式组合使用
    ...
    public:
        /*
         * @brief 构造函数（参数传入版本）
         * @param 模型 频域MoE模型实例指针
         * @param 词汇表大小 输出token词汇表大小
         * @param 最大序列长度 支持的最大生成序列长度
         */
        状态空间递归生成器(std::shared_ptr<频域MoE模型> 模型,
            int64_t 词汇表大小,
    ...
    private:
        std::shared_ptr<频域MoE模型> 模型_;  ///< 组合的频域MoE模型
        int64_t 词汇表大小_;
        int64_t 最大序列长度_;
